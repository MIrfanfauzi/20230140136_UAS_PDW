...
<?php elseif (isset($_GET['status']) && $_GET['status'] === 'laporan_edited'): ?>
    <div class="mb-4 p-4 rounded-md bg-yellow-200 text-yellow-900">
...
<?php if (count($praktikum_list) === 0): ?>
    <div class="p-6 text-yellow-600">Anda belum terdaftar pada praktikum apapun.</div>
<?php else: ?>
    <?php foreach ($praktikum_list as $praktikum): ?>
        <div class="mb-10">
            <!-- Informasi Praktikum -->
            <div class="mb-4">
                <h2 class="text-3xl font-bold text-yellow-900 mb-2">📘 <?php echo htmlspecialchars($praktikum['nama_praktikum']); ?></h2>
                <p class="text-yellow-800"><?php echo htmlspecialchars($praktikum['deskripsi']); ?></p>
            </div>

            <!-- Daftar Modul -->
            <div class="space-y-6">
                <h3 class="text-xl font-semibold text-yellow-700 mb-2">Daftar Modul</h3>
                <?php if (!empty($modul_map[$praktikum['id']])): ?>
                    <?php foreach ($modul_map[$praktikum['id']] as $modul): ?>
                        <div class="bg-yellow-50 p-6 rounded-xl shadow">
                            <h4 class="text-lg font-bold text-yellow-800 mb-2"><?php echo htmlspecialchars($modul['nama_modul']); ?></h4>
                            <p class="text-yellow-600 mb-2">
                                Materi:
                                <?php if (!empty($modul['file_materi'])): ?>
                                    <a href="../uploads/materi/<?php echo htmlspecialchars($modul['file_materi']); ?>" 
                                       class="text-yellow-700 underline hover:text-yellow-900" download>
                                       Unduh Materi
                                    </a>
                                <?php else: ?>
                                    <span class="text-yellow-400">Belum ada file materi</span>
                                <?php endif; ?>
                            </p>

                            <!-- Status Laporan -->
                            <?php if (isset($laporan_data[$modul['id']])): ?>
                                <div class="text-yellow-700 font-medium mb-1">Laporan telah dikumpulkan.</div>
                                <p class="text-yellow-800">Nilai: <strong><?php echo $laporan_data[$modul['id']]['nilai'] ?? 'Belum dinilai'; ?></strong></p>
                                <p class="text-sm text-yellow-600">File: <?php echo htmlspecialchars($laporan_data[$modul['id']]['file_laporan']); ?></p>
                                <div class="flex gap-2 mt-2">
                                    <!-- Tombol Edit -->
                                    <form method="GET" action="" onsubmit="return false;">
                                        <button type="button" onclick="showEditForm('<?php echo $modul['id']; ?>','<?php echo $praktikum['id']; ?>')" class="bg-yellow-600 hover:bg-yellow-800 text-white px-4 py-2 rounded-md font-bold">
                                            Edit
                                        </button>
                                    </form>
                                    ...
                                    <form method="POST" action="" onsubmit="return confirmBatal('<?php echo $modul['id']; ?>');" class="inline">
                                        ...
                                        <button type="button" onclick="showBatalModal('<?php echo $modul['id']; ?>')" class="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-md font-bold">
                                            Batal Pengumpulan
                                        </button>
                                    </form>
                                </div>
                                ...
                                <div id="modal-batal-<?php echo $modul['id']; ?>" class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 hidden">
                                    <div class="bg-yellow-50 rounded-lg p-6 shadow-lg w-full max-w-md">
                                        <h3 class="text-lg font-bold mb-4 text-yellow-700">Konfirmasi Batal Pengumpulan</h3>
                                        ...
                                        <form method="POST" action="batal_laporan.php">
                                            ...
                                            <div class="flex justify-end gap-2">
                                                <button type="button" onclick="closeBatalModal('<?php echo $modul['id']; ?>')" class="px-4 py-2 rounded bg-yellow-200 hover:bg-yellow-300 text-yellow-900">Tidak</button>
                                ...
                                <div id="modal-edit-<?php echo $modul['id']; ?>" class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 hidden">
                                    <div class="bg-yellow-50 rounded-lg p-6 shadow-lg w-full max-w-md">
                                        <h3 class="text-lg font-bold mb-4 text-yellow-700">Edit Laporan</h3>
                                        <form method="POST" action="edit_laporan.php" enctype="multipart/form-data">
                                            ...
                                            <div class="flex justify-end gap-2">
                                                <button type="button" onclick="closeEditModal('<?php echo $modul['id']; ?>')" class="px-4 py-2 rounded bg-yellow-200 hover:bg-yellow-300 text-yellow-900">Batal</button>
                                                <button type="submit" class="px-4 py-2 rounded bg-yellow-700 hover:bg-yellow-800 text-white font-bold">Simpan Perubahan</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            <?php else: ?>
                                ...
                                <form method="POST" action="upload_laporan.php" enctype="multipart/form-data" class="mt-4 space-y-3">
                                    ...
                                    <input type="file" name="file_laporan" required 
                                           class="w-full border border-yellow-300 rounded-md px-4 py-2">

                                    <button type="submit" 
                                            class="bg-yellow-700 hover:bg-yellow-800 text-white font-bold px-4 py-2 rounded-md transition-colors">
                                        Upload Laporan
                                    </button>
                                </form>
                            <?php endif; ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class="text-yellow-500">Belum ada modul pada praktikum ini.</div>
                <?php endif; ?>
            </div>
        </div>
    <?php endforeach; ?>
