...
<!-- Selamat Datang -->
<div class="bg-gradient-to-r from-yellow-500 to-yellow-300 text-yellow-900 p-8 rounded-xl shadow-lg mb-8">
    <h1 class="text-3xl font-bold">Halo, <?php echo htmlspecialchars(strtok($nama_mahasiswa, ' ')); ?> 👋</h1>
    <p class="mt-2 text-yellow-900 text-opacity-80">Selamat datang kembali di SIMPRAK. Semangat terus menyelesaikan tugasmu!</p>
</div>

<!-- Statistik -->
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
    <div class="bg-yellow-50 shadow-md p-6 rounded-xl text-center hover:shadow-lg transition">
        <div class="text-yellow-700 text-4xl font-extrabold"><?php echo $total_praktikum; ?></div>
        <p class="mt-2 text-yellow-800">Praktikum Diikuti</p>
    </div>
    <div class="bg-yellow-100 shadow-md p-6 rounded-xl text-center hover:shadow-lg transition">
        <div class="text-yellow-600 text-4xl font-extrabold"><?php echo $total_selesai; ?></div>
        <p class="mt-2 text-yellow-800">Tugas Selesai</p>
    </div>
    <div class="bg-yellow-200 shadow-md p-6 rounded-xl text-center hover:shadow-lg transition">
        <div class="text-yellow-500 text-4xl font-extrabold"><?php echo $total_menunggu; ?></div>
        <p class="mt-2 text-yellow-800">Tugas Menunggu</p>
    </div>
</div>

<!-- Notifikasi Terbaru -->
<div class="bg-yellow-50 shadow-md p-6 rounded-xl">
    <h2 class="text-2xl font-bold mb-4 text-yellow-900">🔔 Notifikasi Terbaru</h2>
    <ul class="space-y-4">
        <?php if ($notifikasi_list && $notifikasi_list->num_rows > 0): ?>
            <?php while($notif = $notifikasi_list->fetch_assoc()): ?>
                <li class="border-b pb-3 last:border-b-0">
                    <div class="text-yellow-900">
                        Nilai untuk <a href="detail_praktikum.php?id=<?php echo $notif['praktikum_id']; ?>" class="text-yellow-700 font-semibold hover:underline">
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <li class="text-yellow-500">Tidak ada notifikasi terbaru.</li>
        <?php endif; ?>
    </ul>
</div>
