...
<body class="bg-yellow-50">
    <!-- Navigasi Publik -->
    <nav class="bg-yellow-500 shadow-md sticky top-0 z-10">
        <div class="container mx-auto px-6 py-3 flex justify-between items-center"> 
            ...
            <div class="flex items-center space-x-4">
                <?php if (isset($_SESSION['user_id'])): ?>
                    <a href="<?php echo $_SESSION['role']; ?>/dashboard.php" class="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">Kembali ke Dashboard Saya</a>
                ...
                <?php else: ?>
                    <a href="login.php" class="text-yellow-700 hover:text-yellow-900">Login</a>
                    <a href="register.php" class="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">Register</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
    ...
    <div class="container mx-auto p-6">
        ...
        <div class="text-center bg-yellow-100 p-8 rounded-xl shadow-lg mb-8">
            <h1 class="text-4xl font-extrabold text-yellow-900">Katalog Mata Praktikum</h1>
            <p class="text-yellow-700 mt-2 max-w-2xl mx-auto">Temukan dan daftar untuk praktikum yang Anda minati. Mulai perjalanan belajar Anda bersama kami hari ini!</p>
            <form method="GET" action="katalog.php" class="mt-6 max-w-lg mx-auto">
                <div class="flex">
                    <input type="text" name="search" class="w-full px-4 py-3 border border-yellow-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-yellow-600" placeholder="Cari berdasarkan nama praktikum..." value="<?php echo htmlspecialchars($search_query); ?>">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-5 rounded-r-md">Cari</button>
                </div>
            </form>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php if ($all_practicums_result->num_rows > 0): ?>
                <?php while ($praktikum = $all_practicums_result->fetch_assoc()): ?>
                    <div class="bg-white rounded-xl shadow-md p-6 flex flex-col justify-between transform hover:-translate-y-2 transition-transform duration-300">
                        <div>
                            <h3 class="text-xl font-bold text-yellow-900 mb-2"><?php echo htmlspecialchars($praktikum['nama_praktikum']); ?></h3>
                            <p class="text-yellow-700 text-sm h-20 overflow-hidden"><?php echo htmlspecialchars($praktikum['deskripsi']); ?></p>
                        </div>
                        <div class="mt-6">
                            <?php if (isset($_SESSION['user_id']) && $_SESSION['role'] === 'mahasiswa'): ?>
                                ...
                                <?php else: ?>
                                    ...
                                    <div id="modal-<?php echo $praktikum['id']; ?>" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
                                        <div class="bg-white rounded-lg p-6 shadow-lg w-full max-w-md">
                                            <h3 class="text-lg font-bold mb-4 text-yellow-700">Konfirmasi Pendaftaran</h3>
                                            ...
                                            <form id="form-daftar-<?php echo $praktikum['id']; ?>" onsubmit="return daftarPraktikumAjax(event, '<?php echo $praktikum['id']; ?>');">
                                                ...
                                                <div class="flex justify-end gap-2">
                                                    <button type="button" onclick="closeModal('<?php echo $praktikum['id']; ?>')" class="px-4 py-2 rounded bg-yellow-200 hover:bg-yellow-300 text-yellow-900">Tidak</button>
                                                    <button type="submit" class="px-4 py-2 rounded bg-yellow-700 hover:bg-yellow-800 text-white font-bold">Iya</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                                <a href="login.php" class="w-full block text-center bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300">
                                    Login untuk Daftar
                                </a>
                            <?php endif; ?>
                        </div>
            ...
            <?php else: ?>
                <div class="col-span-full text-center py-12 bg-white rounded-lg shadow-md">
                    <svg class="mx-auto h-12 w-12 text-yellow-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    <h3 class="mt-2 text-lg font-medium text-yellow-900">Tidak Ditemukan</h3>
                    <p class="mt-1 text-sm text-yellow-700">Tidak ada mata praktikum yang sesuai dengan pencarian Anda.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
